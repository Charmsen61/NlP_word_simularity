...
def build_cooccurrence_matrix(tokens, vocab, window_size=2):
    ...
    for i, target_word in enumerate(tokens):
        ...
        #examine words inside the context window
        for j in range(start,end):
            #ignor the word itself
            if i == j:
                continue
...
def word_to_vector(word, vocab, matrix):
    ...
    for context_word in vocab:
        vector.append(matrix[context_word][word])
...
def most_similar_words(target_word, vocab, matrix, top_n=3):
    ...
    for word in vocab:
        ...
        comparison_vector = word_to_vector(word, vocab, matrix)

        similarity = cosine_similarity(target_vector, comparison_vector)
